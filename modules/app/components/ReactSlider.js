(function (root, factory) {
  ...
  var ReactSlider = React.createClass({
    ...
    getInitialState: function () {
      ...
      // this.tempArray = value.slice(0, value.length - 1)
    ...
    isExceedingMax: function (value) {
      var max = this.props.max
      var sum = value.reduce((prev, curr) => prev + curr)
      return sum > max
    },
    ...
    componentWillReceiveProps: function (newProps) {
      ...
      if (this.isExceedingMax(value)) {
        return
      }
      for (var i = 0; i < value.length; i++) {
        value[i] = this._trimAlignValue(value[i], newProps);
      }
      ...
      isValueChanged = isValueChanged || (value.length !== this.state.value.length)
    ...
    _buildHandleStyle: function (offset, i) {
      var pos = this._accumulate(offset.slice(0, i + 1));
      var style = {
        position: 'absolute',
        willChange: this.state.index >= 0 ? this._posMinKey() : '',
        zIndex: this.state.zIndices.indexOf(i) + 1
      };
      style[this._posMinKey()] = pos + 'px';
      return style;
    },
    ...
    _getClosestIndex: function (pixelOffset) {
      ...
      for (var i = 0; i < l; i++) {
        var offset = this._calcOffset(this._accumulate(value.slice(0, i + 1)));
    ...
    _calcOffsetValue: function (previousValues, currentValue) {
      var totalPreviousValue = this._accumulate(previousValues)
      return currentValue - totalPreviousValue;
    },
    ...
    _forceValueFromPosition: function (position, callback) {
      ...
      value[closestIndex] = this._calcOffsetValue(value.slice(0, closestIndex), nextValue);
      var valueChange = value[closestIndex] - this.state.value[closestIndex]
      // value[closestIndex + 1] = value[closestIndex + 1] - valueChange;
      value = this.adjustValue(value, closestIndex, valueChange)
      if (!this.checkMinDistance(value)) {
        return
      }
      this.setState({value: value}, callback.bind(this, closestIndex));
    },

    checkMinDistance(value) {
      // Prevents the slider from shrinking below `props.minDistance`
      var length = this.props.ignoreLastMinDistance ? value.length - 1: value.length
      for (var i = 0; i < length; i += 1) {
        if (value[i] < this.props.minDistance) return;
      }
      if (this.props.ignoreLastMinDistance && (value[length] < 0)) return;
      return true;
    },
    ...
    adjustValue: function (value, index, change) {
      var absChange = Math.abs(change);
      console.log(this.isExceedingMax(value))
      if ((change > 0) && this.isExceedingMax(value)) {
        for (let i = index+1; i < value.length; i++) {
          let currVal = value[i]
          value[i] = Math.max(currVal - absChange, 0)
          absChange = Math.max(absChange - currVal, 0)
        }
      }
      if (change < 0) {
        value[index + 1] = Math.min(value[index + 1] + absChange, 100)
        if(value[index] === 0) {
    ...
    _move: function (position) {
      ...
      if (newValue !== oldValue) {
        ...
        value = this.adjustValue(value, index, change)
        console.log(value)
        if (!this.checkMinDistance(value)) {
          return
        }
        ...
        var newState = {value: value.slice()}
        if ((diffValue < 0) && (newValue === 0) && (index > 0)) {
          newState.index = index - 1
          newState.startValue = value[index - 1]
        }
        this.setState(newState, this._fireChangeEvent.bind(this, 'onChange'));
      }
    },
    ...
    _renderHandles: function (offset) {
      ...
      for (var i = 0; i < length; i++) {
        styles[i] = this._buildHandleStyle(offset, i);
      }
    ...
    _renderBar: function (i, offsetFrom, offsetTo, val) {
      return (
    ...
    _renderBars: function (offset) {
      ...
      bars.push(this._renderBar(0, 0, offset[0], value[0]));
      for (var i = 0; i < lastIndex; i++) {
        bars.push(this._renderBar(i + 1, this._accumulate(offset.slice(0, i+1)), this._accumulate(offset.slice(0, i+2)), value[i + 1]));
      }
      bars.push(this._renderBar(lastIndex + 1, this._accumulate(offset.slice(0, lastIndex + 1)), this.state.upperBound, value[lastIndex + 1]));
      return bars;
    },
